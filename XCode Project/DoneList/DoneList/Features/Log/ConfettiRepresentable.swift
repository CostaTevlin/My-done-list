...
// Inlined from unionst/union-confetti with birthRate reduced 100 → 15.
...
class ConfettiUIView: UIView {
    ...
    lazy var confettiTypes: [ConfettiType] = {
        let confettiColors: [UIColor] = [
            (149, 58, 255), (255, 195, 41), (255, 101, 26),
            (123, 92, 255), (76, 126, 255), (71, 192, 255),
            (255, 47, 39),  (255, 91, 134), (233, 122, 208)
        ].map { UIColor(red: $0.0 / 255, green: $0.1 / 255, blue: $0.2 / 255, alpha: 1) }

        return [ConfettiPosition.foreground, .background].flatMap { position in
            [ConfettiShape.rectangle, .circle].flatMap { shape in
                confettiColors.map { ConfettiType(color: $0, shape: shape, position: position) }
            }
        }
    }()
    ...
    class ConfettiType {
        ...
        lazy var image: UIImage = {
            let rect: CGRect = shape == .rectangle
            ...
            shape == .rectangle ? ctx.fill(rect) : ctx.fillEllipse(in: rect)
    ...
    enum ConfettiShape { case rectangle, circle }
    enum ConfettiPosition { case foreground, background }
}
